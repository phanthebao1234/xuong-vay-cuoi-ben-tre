...
async function getProduct(slug: string): Promise<ProductLookup> {
  try {
    ...
    // A valid product from another category must not silently render under /suits.
    if (product.category_slug !== SUIT_CATEGORY_SLUG) {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = product.description
    ...
    : `${product.name} — thiết kế ${product.category_name ?? 'vest cưới'} tại Xưởng Váy Cưới Bến Tre. Đặt lịch thử trực tiếp tại showroom.`
  ...
  return {
    title: product.name,
    description,
    alternates: { canonical: `/suits/${product.slug}` },
    openGraph: {
      title: product.name,
      description,
      type: 'website',
...
export default async function SuitDetailPage({ params }: PageProps) {
  const { slug } = await params
  ...
  return (
    <ProductDetail
      ...
      breadcrumbFallbackLabel="Vest cưới"
